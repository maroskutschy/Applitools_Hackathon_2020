...
#    Then I make Applitools Visual Test validation running via UltraFast Grid with test name: "Task 3" and step name: "Product Details test"
  ...
  Scenario Outline: AppliFashion Traditional - all 3 tasks
    ...
    And I click on item number "1" in the selection
    And I validate that selected value of size option is "Small (S)" for task No "3" and "<browser>" browser and "<width>" viewport width and "<height>" viewport height and "<device type>" device type
    And I validate that value of new price is "$33" for task No "3" and "<browser>" browser and "<width>" viewport width and "<height>" viewport height and "<device type>" device type
